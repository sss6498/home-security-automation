/*
This code is created for the purpose of the demo. This
When run, the processing file will display a dialog box 
that will allow the user to enter in commands. The user
can enter a capture command, which will capture an image
from the arducam, and save it in a folder. It also allows 
to change the size of the picture from the arducam. This 
code can only be run if the arducam is connected to the 
computer and the CaptureCode.ino file is already uploaded
to the arduino.
*/
